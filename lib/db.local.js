...
function execQuery(sql, params) {
  ...
  if (upper.startsWith('INSERT')) {
    ...
    const valueM = s.match(/VALUES\s*\((.+)\)/i);
    if (!tableM || !valueM) return [{ insertId: 0 }, []];

    const table = tableM[1];
    const cols = tableM[2].split(',').map(c => c.trim());
    // Split com regex simples pode falhar com vírgulas dentro de aspas, mas para as nossas queries simples servirá bem.
    // Usaremos split por vírgula mantendo string parsing se necessário, ou melhor: simplificando:
    const valsRaw = valueM[1].split(',').map(v => v.trim());
    ...
    let paramIdx = 0;
    cols.forEach((col, i) => {
      const valStr = valsRaw[i];
      if (valStr === '?') {
        row[col] = params[paramIdx++];
      } else {
        // Assume que é um literal numérico ou string com aspas simples ('Pendente')
        row[col] = valStr ? valStr.replace(/^['"]|['"]$/g, '') : null;
      }
    });
